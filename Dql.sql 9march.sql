...
Select employeenumber Empid,lastname LastName, firstname FirstName from employees;
Select employeenumber 'Emp ID',lastname 'Last Name',firstname 'First Name' from employees;
Select customerNumber 'Customer ID',customerName 'Customer  Name',contactLastName 'Last Name' from customers;

Select distinct jobTitle from employees;
Use Dql;
select firstname, officecode from employees;
select firstname, officecode from employees where officecode =4;
select firstname, officecode from employees where officecode >4;
select firstname, officecode from employees where officecode >=4;
select firstname, officecode from employees where officecode <4;
select firstname, officecode from employees where officecode <=4;
...
Select CustomerName,Creditlimit,City from customers where creditlimit >100000;
Select CustomerName,Creditlimit,City from customers where creditlimit <100000;
Select CustomerName,Creditlimit,City from customers where city = 'NYC';

Select * from products;
Select productName,productLine from products where productline = 'motorcycles';

Select customername, city ,creditlimit from customers where creditlimit > 100000 and city = 'NYC';
Select customername, city ,creditlimit from customers where creditlimit > 100000 or city = 'NYC';
 -- before applying a condition first i have to check.
 Select * from employees;
 Select firstName,jobTitle from employees where officeCode = 4 and jobTitle = 'sales rep';
 Select firstName,jobTitle from employees where officeCode = 4 or jobTitle = 'sale rep';
 Select productname,productline,productscale from products where productline = 'motorcycles' and productscale = '1:10';
Select firstName,jobTitle,officecode from employees where officecode=4 or jobTitle  = 'sales rep';
select checkNumber , amount from payments where amount between 10000 and 30000;
/* Between operator*/
Select * from orders;
select orderNumber, orderdate from orders where orderdate between '2004-10-01' and '2004-12-31';
/* Not operator*/
select customername,creditlimit from customers where not creditlimit between 100000 and 200000;
/* In operator*/
select firstname, officecode from employees where officecode in (2,4,6);
select firstname, officecode from employees where officecode = 4 or officecode = 2 or officecode = 6;

select customername, country from customers where country in ('USA','UK','USA','France');

-- Like operator

Use Dql;

Select firstname,lastname from employees where firstname like 'A%';-- First letter 
Select firstname, lastname from employees where firstname like '%Y'; -- last letter

Select * from customers;
Select customerName  from customers where customerName like 'T%';
Select customerName  from customers where customerName like '%S';
Select firstname  from employees  where firstname like '%I%';

Select customerName  from customers where customerName like '%A%';
Select customerName  from customers where customerName like '%A%' and customerName like '%I%'; -- both will satisfy 
Select customerName  from customers where customerName like '%A%' or customerName like '%I%'; 

-- Underscore
select firstname from employees where firstname like 'M___';

select * from products;
Select productName from products where productName like 'The________';

select * from orders;
Select orderdate, orderNumber from orders where orderdate like '2004%';
Select orderdate, orderNumber from orders where orderdate like '2004-11%';
Select orderdate, orderNumber from orders where orderdate like '2004-12%';

Select * from payments;
show tables;

Select paymentDate,checkNumber from payments where paymentDate like '-04-%';

-- Is null Operator 
select customername,state from customers where state is null;
select customername,addressline2 from customers where addressline2 is null;
select customername,addressline2 from customers where addressline2 is not null;

-- Sorting
select firstname,lastname from employees order by firstname;
select customername from customers order by state;
select productname,productline,productscale from products  order by productline,
productscale;

Select * from customers;
select customerName,city,country from customers order by city,country;


select customerName,creditlimit from customers order by creditlimit desc;
select firstname from employees order by firstname desc;
select customerName,creditlimit from customers where creditlimit>100000 order by creditlimit desc;
 -- Limiting
 Use Dql;
 Select customerName,creditlimit from customers where creditlimit>100000 order by creditlimit desc limit 5;
 Select customerName,creditlimit from customers where creditlimit>100000 order by creditlimit desc limit 5 offset 5;
 
 -- Case Clause
 Select * from customers;
 select customerName,country from customers;
 
 select customerName,country,case country when 'USA' then 'Free delivery' else '5% Delivery charge'
 end 'Message' from customers;
 
Select customerName,country,case country when 'USA' then 'Free delivery' when 'Uk' then '2% delivery charg'
when 'France' then '3% delivery charge' end 'Message' from customers;
-- BuilT in Functions
Select * from employees;
Select firstName,lastName,concat(firstName,lastName) from employees;
Select firstName,lastName,concat(firstName,lastName)'Full Name' from employees;
Select firstName,lastName,concat(firstName,'  ',lastName) 'Full Name' from employees;
Select * from customers;
Select customerName,city,concat(customerName,'   ','is located in','  ',city)'Full Name and city' from 
customers;
-- Upper function
Select upper(firstname),upper (lastName) from employees;

-- Lower function
Select lower(firstName),lower(lastName) from employees;
-- lenght
select firstName,length(firstName) from employees order by length(firstName);
select firstName,length(firstName) from employees where length(firstName)=6;

select customerName,city,upper(concat(customerName, '  ', 'is located in','  ',city))'Location'
from customers;
-- Replace
Select * from employees;
select firstName,email,replace(email,'classicmodelcars.com','gmail')'Replaced Mail' from employees;
-- update comment
Update employees set email = replace(email,'classicmodelcars.com','gmail') where officeCode = 1;
Select * from employees;

-- Substr
select firstName ,substr(firstName,1,2)from employees;
Select * from customers;
show tables;
select customerName,substr(customerName,1,2) 'Messsage'from customers;
 -- Date function
 select curdate();
 select now();
 select sysdate();
select month ('2022-12-01'); -- it only show month
select * from orders;
select orderNumber,orderDate,month(orderDate)'Rmark' from orders;
select orderNumber,requiredDate,month(requiredDate)'Remark' from orders;
select orderNumber,requiredDate,year(requiredDate)'Remark' from orders; -- it only show year
select orderNumber,orderDate,month(orderDate),year(orderDate) from orders where month (orderDate)=2
and year(orderdate)=2005;
select orderNumber,requiredDate,month(requiredDate),year(requiredDate) from orders where month (orderDate)=3
and year(orderdate)=2004;
select ordernumber,orderdate,last_day(orderdate) from orders;
select ordernumber,orderdate,adddate(orderdate,5) from orders;-- it do addition
select * from orders;
alter table orders add column delivery_date date after orderdate;
alter table orders add column deliverydate date after orderNumber;
select ordernumber,orderdate,shippeddate,datediff(orderdate,shippeddate) from orders;
select datediff('2023-02-23','1992-07-19');
select ordernumber,orderdate,timestampdiff(year,orderdate,sysdate())from orders;
select ordernumber,orderdate,timestampdiff(month,orderdate,sysdate())from orders;
select date_format(sysdate(),'%m');
select orderdate ,date_format(orderdate,'%m') from orders;
select orderdate ,date_format(orderdate,'%y') from orders;
select orderdate ,date_format(orderdate,'%Y') from orders;
select orderdate ,date_format(orderdate,'%b') from orders;
select orderdate ,date_format(orderdate,'%M') from orders;
select orderdate ,date_format(orderdate,'%W') from orders;
select orderdate ,date_format(orderdate,'%a') from orders;
select orderdate ,date_format(orderdate,'%D') from orders;
select orderdate ,date_format(orderdate,'%h') from orders;
select orderdate ,date_format(orderdate,'%i') from orders;
select orderdate ,date_format(orderdate,'%s') from orders;
select orderdate ,date_format(orderdate,'%p') from orders;

select orderdate ,date_format(orderdate,'%D of the %M,%Y,%W') 'Result'from orders;
select orderdate ,date_format(orderdate,'%d-%m-%Y') 'Result' from orders;
alter table orders add column newdateformat date after orderdate;
select * from orders;
update orders set newdateformat = date_format(orderdate,'%d-%m-%y');
alter table orders drop column newdateformat;

Use Dql;
/*Numeric function*/
select * from payments;
Select customerNumber,amount,ceil(amount)from payments; -- Round off to the higest
select customerNumber,amount,floor(amount)from payments; -- 
select customerNumber,amount,sqrt(amount)from payments; 
select customerNumber,amount,sqrt(9)from payments; 
select customerNumber,amount,sqrt(81)from payments; 
select customerNumber,amount,sqrt(16)from payments; 
select customerNumber,amount,pow(1,3)from payments; 

-- Aggregate function.
Select * from customers;
select customerName,Country,sum(creditLimit) from customers;
select customerName,Country,min(creditLimit) from customers;
select customerName,Country,max(creditLimit) from customers;
select customerName,Country,avg(creditLimit) from customers;
select customerName,Country,count(creditLimit) from customers;
...
select avg(distinct creditLimit) from customers;
select count(creditLimit) from customers;

select country,sum(creditlimit) from customers where country = 'USA';
select country,count(creditlimit),sum(creditLimit),min(creditLimit),max(creditlimit),avg(creditLimit)
from customers where country = 'USA';

select country,count(creditlimit),sum(creditLimit),min(creditLimit),max(creditlimit),avg(creditLimit)
from customers where country in ('USA','UK','France');
...
select country,count(creditlimit),sum(creditLimit),min(creditLimit),max(creditlimit),avg(creditLimit)
from customers group by country order by sum(creditLimit)desc;

select country,count(creditlimit),sum(creditLimit),min(creditLimit),max(creditlimit),avg(creditLimit)
from customers group by country having country in ('USA','France','UK');

select country,count(creditlimit),sum(creditLimit),min(creditLimit),max(creditlimit),avg(creditLimit)
from customers group by country having country in ('USA','France','UK','German') order by count(creditLimit)
desc;
select * from payments;
select customerNumber,count(amount),sum(amount) from payments group by customerNumber;

select customerNumber,count(amount),sum(amount) from payments group by customerNumber having
customerNumber in (124,145,121)order by count(amount) desc;
...
select firstname,officecode from employees where officecode =
(select officecode from employees where firstname = 'Mary');

select * from customers;
select customerName,country from customers where country
 =(select country from customers
where customerName = 'FUNGIFTIDEAS.COM');

Select avg(creditLimit) from customers;
Select CustomerName,Creditlimit,City from customers where creditlimit >67660;
select customerName,creditlimit from 
customers where creditlimit >(select avg(creditlimit) from customers);
...
select customerName,creditlimit from customers
 ...
 select customerName,creditlimit,city from customers
 where creditlimit>(select max(creditlimit) from customers where city='NYC');
 
 -- Multi Row operator
select customername,country,creditlimit from customers where creditlimit >all(select avg(creditlimit)
from customers group by country having country in ('Germany','France'));

Select productname,productvendor,productline from products where productvendor in 
(select productvendor from products where productline = 'PLanes'); 

select productname,msrp,productline from products where msrp <all
(select (msrp)from products where productline = 'Motorcycles');

select * from employees;
select jobTitle,officecode,firstname from employees where (jobtitle,officecode)=
(select jobTitle,officecode from employees where firstname = 'Pamela');

select * from customers;
select country,salesRepEmployeeNumber from employees where ( country,salesRepEmployeeNumber)=
(select  country, salesRepEmployeeNumber

from customers where customerName = 'Atelier graphique'); -- make it correct

select * from employees;
create table empnew(name varchar(20),officecode int,jobtitle varchar(30));
insert into empnew select firstname,officecode,jobtitle from employees;
select * from empnew;

select * from customers;
create table customer1 (custnum int,custname varchar (40),custcountry varchar(30),creditlimit int);
insert into customer1 select customerNumber,customerName,country,creditLimit from customers;
select * from customer1;

select * from customers; -- Short way to create 
Create table USAcustomer as select customername,country,creditlimit from customers where country = 'USA';
Select * from USAcustomer; 	

Select * from products;
Drop table productline;
Create table productline as select productName,productLine from products where productLine = 'Classic Cars';
Select * from productline;

-- Update subquery
Select * from customers;
update customers set creditlimit = creditlimit + 1000 where city = 
(select city from customers where customername = 'FunGiftIdeas.com');
select * from customers where city ='New Bedford';

update customers set creditlimit = creditlimit + 1000 where country =
(select country from customers where customername = 'FunGiftIdeas.com');
...
-- Join 
-- Cross join
Select firstname,city from employees,offices;

/* Inner join in inner where condition is compulsory*/ 
select firstname, city,employees.officecode from employees,offices where  employees.officecode = offices.officecode;
select firstname, city,e.officecode from employees e,offices  o where  e.officecode = o.officecode;
Select firstname,lastname,employees.officeCode from employees ,offices where employees.officeCode = offices.officeCode;
Select firstname,lastname,employees.officecode from employees,offices where employees.officecode = offices.officecode;
Select firstname,lastname,employees.officecode from employees,offices where employees.officecode = offices.officecode;
/*on*/
select firstname,city,e.officecode from employees e join offices o on(e.officecode=o.officecode);
select firstname,city,e.officecode from employees e join offices o using(officecode);

Select * from employees;
Select  * from customers;
Select customername,firstname,salesrepemployeenumber from customers ,employees 
 where salesrepemployeenumber=employeenumber;
  /* Non equi join*/
  
  Select * from customers;
  
Create table creditgrade(lowvalue decimal(10.2),highvalue decimal(10.2),Grade varchar(20));
Show tables;
Describe creditgrade;
Insert into creditgrade(lowvalue,highvalue,Grade)values(0,25000,'bronze'),(25001,50000,'Silver'),
(50001,75000,'Gold'),(75001,100000,'Diamond'),(10001,300000,'Platinum');
Select * from creditgrade;
/* on we use where we have to take*/
Select customername,creditlimit,Grade from customers join creditgrade on (creditlimit between lowvalue
and highvalue);-- between finding under  values
/* Left join*/
select * from customers;
select * from employees;
Select customername,salesrepemployeenumber,firstname from customers left outer join employees 
on(salesrepemployeenumber=employeenumber);

  Select customername,salesrepemployeenumber,firstname from customers right outer join employees 
on(salesrepemployeenumber=employeenumber);
update customers set salesrepemployeenumber= 1143 where customername='Havel & Zbyszek Co';
update customers set salesrepemployeenumber =1143 where customername in('Porto Imports Co.',
'Asian Shopping Network, Co','Natürlich Autos','ANG Resellers');

/* Self join*/
Select * from employees; -- Export this file 

create table p1(sid int ,sname varchar(20),lid int);
insert into p1 values(1,'Prabhakar',null),(2,'Yadnya',null),(3,'Deepa',null),
(4,'Deepanjali',1),(5,'suraj',3),(6,'Sushil',2),(7,'Ganesh',1),(8,'Siddhesh',3),
(9,'Dinesh',2),(10,'Ayush',1),(11,'Aditya',2),(12,'Ninad',3);
Select * from p1;
Select St.sname,Ld.sname,Ld.lid from p1 ST join p1 LD on(Ld.lid=ST.sid);
/* We give alias name due to no confusssed our mysql*/
Select Emp.firstname,MNGR.firstname from employees EMP join employees MNGR 
on(MNGR.employeenumber=EMP.reportsto);
select *from employees;
Create table sports(Pid int,Player varchar(30),Lid int);
Insert into sports values(1,'Ayush',null),(2,'Rohit',null),(3,'Suraj',2),
(4,'yadnya',1),(5,'Deepa',1),(6,'Suraj',2),(7,'Ganesh',1),(8,'Ramesh',3),
(9,'Pankaj',4),(10,'Jay',3);
Select * from sports;
Select ts.Player,sp.Player from sports sp join sports ts on (
Ts.pid = sp.lid);
Create table Cinema(Did int,Movie varchar(30),ReportDid int);
Insert into Cinema values(1,'josh',9),(2,'Abc',10),
(3,'Mohan',8),(4,'Masoom',10),(5,'Fighter',8),(6,'Aryan',9),(7,'Mauj',10),(8,'Kal ho na ho',Null),
(9,'Kbkg',Null);
Select * from cinema;
Select m.Movie,w.Movie from Cinema m join cinema w on (m.Did  = w.ReportDid);

/* Views*/
Select * from employees;
Select employeenumber,firstname,lastname,officecode from employees where officecode = 4;
Create view officecode4 as select employeenumber,firstname,lastname,officecode 
from employees where officecode = 4;
Select * from officecode4;
Drop view officecode4;

-- Create view officecode5 as select officecode where officecode in (1,2,3) from employees; -- pending
/* Create view emp*/
Create view empoffice as select firstname,city,emp .officecode from employees emp,offices  o where
(emp.officecode = o.officecode);
select * from empoffice;

Select * from customers;
create view salesrep as select customername,firstname,salesrepemployeenumber from customers,
employees where salesrepemployeenumber = employeenumber;
Select * from salesrep;

Create view officecode6 as select employeenumber,firstname,lastname,
officecode from employees where officecode =6;
select * from officecode6;
insert into employees(employeenumber,firstname,lastname,officecode) values(1411,'Jinal','Thakur',6),
(1502,'Jinal','Tr',6);
Select * from employees;
